...
class OgbInfo:
    def __init__(self, dataset_name, root) -> None:
        self.dataset_name = dataset_name
        self.root = root


class DataFactory():
    def __init__(self, dataset=None, ogb_info: OgbInfo = None, device='cpu', include_year=False):
        self.device = device
        self.include_year = include_year
        self.mask_rate = 0.5
        self.dataset = dataset
        if not dataset:
            self.dataset = DglNodePropPredDataset(ogb_info.dataset_name, ogb_info.root)
