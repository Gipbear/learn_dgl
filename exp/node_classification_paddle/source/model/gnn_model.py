...
class GAT(nn.Module):
    def __init__(self, in_size: int, hid_size: int, out_size: int, heads: list[int]) -> None:
        ...
        self.gat_layers.append(
            dglnn.GATConv(in_size, hid_size, heads[0], feat_drop=0.3, attn_drop=0.1, residual=True, activation=F.relu)
        )
        self.gat_layers.append(
            dglnn.GATConv(hid_size * heads[0], hid_size, heads[1], feat_drop=0.3,
                          residual=True, attn_drop=0.1, activation=F.relu)
        )
        self.gat_layers.append(
            dglnn.GATConv(hid_size * heads[1], out_size, heads[2], feat_drop=0.3,
